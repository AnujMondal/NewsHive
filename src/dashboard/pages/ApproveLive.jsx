...
const ApproveLive = () => {
    ...
    try {
      if (localStorage.getItem("user")) {
        const res = await axios.get(
          `https://newshive-express-1.onrender.com/indiNewsData`,
          {
            headers: {
  ...
  const statusHandel = async(id,s)=>{
        const res = await axios.post("https://newshive-express-1.onrender.com/liveStatusChange",{_id:id,status:s,e:localStorage.getItem("name")});
        if(res.status == 200)
        {
            setRefresh((refresh)=>{let temp = refresh;
                temp = !temp;
                return temp;
            });
        }
  ...
  return (
    <div className="p-6 min-h-screen bg-gradient-to-br from-white to-blue-50">
      <div className="max-w-5xl mx-auto">
        <h2 className="text-2xl font-bold mb-6 text-gray-800">News Live</h2>
              ...
              {dd.map((news, idx) => (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={()=>{statusHandel(news._id,news.status);}}
                    >
                    {news.status}
                    </button>
                  </td>
